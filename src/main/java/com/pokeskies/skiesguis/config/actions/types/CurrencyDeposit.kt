...
class CurrencyDeposit(
    ...
    override fun executeAction(player: ServerPlayer) {
        ...
        if (service == null) {
            Utils.printError("[ACTION - CURRENCY_DEPOSIT] No Economy Service could be found from '$economy'! Valid services are: ${SkiesGUIs.INSTANCE.getLoadedEconomyServices().keys}")
            return
        }
    ...
    override fun toString(): String {
        return "CurrencyDeposit(click=$click, delay=$delay, chance=$chance, requirements=$requirements, " +
                "currency=$currency, amount=$amount, economy='$economy')"
    }
}
