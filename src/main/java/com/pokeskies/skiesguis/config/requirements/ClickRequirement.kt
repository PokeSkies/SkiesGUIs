    ...
    companion object {
        val CODEC: Codec<ClickRequirement> = RecordCodecBuilder.create { instance ->
            instance.group(
                ...
                Codec.unboundedMap(Codec.STRING, SubAction.CODEC).optionalRecordCodec("deny_actions", ClickRequirement::denyActions, emptyMap()),
                Codec.unboundedMap(Codec.STRING, SubAction.CODEC).optionalRecordCodec("success_actions", ClickRequirement::successActions, emptyMap()),
            ).apply(instance) { requirements, denyActions, successActions ->
                ClickRequirement(requirements, denyActions, successActions)
            }
        }
    }

    override fun toString(): String {
        return "ClickRequirement(requirements=$requirements, denyActions=$denyActions, successActions=$successActions)"
    }
}